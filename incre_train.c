...
const int vocab_hash_size = 80000000;  // Maximum 60 * 0.7 = 42M words in the vocabulary
const int new_word_hash_size = 20000000;  // Maximum 30 * 0.7 = 21M words in the new word array
...
char embedding_file[MAX_STRING], update_list[MAX_STRING];
...
// This is a upgraded version of read sentence.
// Reads a sentence from a file, assuming each line in the file is a sentence
// return sentence length
// sen_length = -1 when hit the end of file
...
// Reads a sentence from a file, assuming each line in the file is a sentence
// This is a simple version with low tolerant to wrong format
// 1. can not handel multiple spaces
// 2. can not handel long words > 100
// 3. can not split sentences with white space at the end.
void GetSentence(char** sentence, FILE *fin) {
    char ch;
    int sen_length = 0;
    while(!feof(fin)) {
        fscanf(fin, "%s", sentence[sen_length]);
        ch = fgetc(fin);
        //       printf("%s ", sentence[sen_length]);
        if (ch == 10) return;
        sen_length ++;
    }
}
...
void SortVocab() {
    ...
    for (a = 0; a < size; a++) {
        ...
        if ((vocab[a].cn < min_count) && (a != 0)) {
            ...
            if(vocab[a].cn < 10) vocab[a].update = 1;
        }
    }
...
// The complexity of sort is nw^2m^2 where n is num_of_new_words, w is window_size, m is the average appearence time of each new word
// However, if insert word each time when adding word to context, the complexity is only nmw^2
// So we do not sort here, rather insert when adding.
// Sorts the context words for each new word
void SortContext() {
    long long a;
    for (a = 0; a < num_of_new_words; a++) {
        char** list = new_words[a].context;
        int length = new_words[a].context_size;
        int current;
        for (current = 0; current < length; current++) {
            char* word = list[current];
            int compare_to;
            for (compare_to = 0; compare_to < length; compare_to++) {
                if(current == compare_to) continue;
                char* word_c = list[compare_to];
                if(strcmp(word, word_c) == 0) {
                    // remove one redandent record
                }
            }
        }
    }
...
void LearnVocabFromTrainFile() {
    ...
    while (sen_length != -1) {
        ...
        //for (j = 0; j < MAX_SENTENCE_LENGTH; j++) free(sentence[j]);
        //for (j = 0; j < MAX_SENTENCE_LENGTH; j++) sentence[j] = malloc(MAX_STRING);
...
void ReadUpdateList() {
    ...
    while (1) {
		//may cause stack smashing if the length of word is not checked
        //fscanf(fin, "%s\n", word);
        ReadWord(word, fin);
        if (feof(fin)) break;
  ...
  // not </s>, random init
    if (strcmp(word, "</s>")){
		...
		for (b = 0; b < layer1_size; b++) {
			...
			syn0[b] = (((next_random & 0xFFFF) / (real)65536) - 0.5) / layer1_size;
			}
	}
		index = SearchVocab(word);
		for(i = 0; i < layer1_size; i++) {
...
//		}
		...
		if(find == 0 && length < 50) 	printf("cannot find word %s in vocabulary\n", word);
    }
    fclose(fin);
    if (debug_mode > 0) {
    ...
    // For debugging, print out the embeddings after load
	if (eval == 2) {
...
int AssignAverage(int a) {
    ...
    if (index == -1) {
        int length = strlen(word);
        if (length < 50) {
			printf("Can not find word %s in the new word list.\n", vocab[a].word);
        printf("word index: %d \n", a);}
        return 0;
    }
...
void InitNet() {
    ...
    // print out the embeddings after initialized for debugging
    if (eval == 2) {
        ...
        if (cbow) {  //train the cbow architecture
            ...
            if (cw) {
                ...
                if (hs) for (d = 0; d < vocab[word].codelen; d++) {
                        ...
                        //if (vocab[vocab[word].point[d]].update == 1) {
...
void TrainModel() {
    ...
    // init the vocab_hash here incase we don't have vocab file
    for (a = 0; a < vocab_hash_size; a++) vocab_hash[a] = -1;
    vocab_size = 0;
    if (read_vocab_file[0] != 0) ReadVocab();
    else AddWordToVocab((char *)"</s>");
    // Training file is the must have
    LearnVocabFromTrainFile();
    if (save_vocab_file[0] != 0) SaveVocab(save_vocab_file);
    if (output_file[0] == 0) return;
    if (update_list[0] != 0) ReadUpdateList();
    // Allow not given pre-trained embedding file. Will behave similar to 
    // original word2vec
    if (embedding_file[0] != 0) {
		LoadEmbeddings();
		InitNet();
	} else {
		if(layer1_size != 0)NormalInit();
		else {
			printf("Error, vector size is unknown: please give a pre-trained modle or assign a vector size using parameter -size\n");
		}
	}
    ...
    if (updateAll == 1) printf("Updating all vocabs\n");
        ...
        if (updateNew ==  1) {
            ...
            else printf("Updating new vocabs\n");
        }
        if (update_list[0] != 0) {
            if (word_in_list == 0) printf("Note: no vocabs in update list\n");
            else printf("Updating vocabs in update list\n");
        }
    }
